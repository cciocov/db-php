...
class db_mysql {
	...
	public function query($qstr) {
		return new db_mysql_query($this, $qstr);
	}
	...
	public function get_field($field, $tbname, $where_str = '') {
		$qstr = "SELECT $field FROM $tbname" . (!empty($where_str) ? " WHERE $where_str" : '') . ' LIMIT 1';
		$q = $this->query($qstr);
	 ...
	 */
	public function get_records($fields, $tbname, $where_str = '', $key_field = '', $callback = false) {
...
class db_mysql_query {
	 ...
	 */
	public function __construct(db_mysql &$db, $qstr, $strict = false) {
		$this->_db = $db;
		$this->_result = @mysql_query($qstr, $this->_db->link_id);
